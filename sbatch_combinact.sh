...
#SBATCH --output=logs_new/wrn_50_rs1b/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=wrn_50_rs1b
...
SAVE_PATH=~/vector_projects/outputs/wrn_50_rs1b
...
#python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_PATH --model resnet --batch_size 128 --actfun relu --resnet_width 2 --optim onecycle --num_epochs 100 --dataset $DATASET --aug --validation --search --mix_pre_apex --label $LABEL
