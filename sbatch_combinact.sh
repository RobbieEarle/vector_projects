...
#SBATCH --output=logs/e9_swish_nparam/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=e9_swish_nparam
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model mlp --dataset mnist --actfun swish --k 1 --var_n_params_log
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model cnn --dataset mnist --actfun swish --k 1 --var_n_params_log