...
#SBATCH --array=32-47%47                         # array value
#SBATCH --output=logs/rms4_grid_relu/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=rms4_grid_relu
...
python engine.py --seed 55 --save_path $SAVE_PATH --check_path $CHECK_DIR --optim rmsprop --model resnet --dataset cifar10 --actfun $ACTFUN --num_epochs 350 --grid_id $GRID_ID --validation --label $GRID_ID
