...
#SBATCH --output=logs/th1_nparam4/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=th1_nparam4
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model mlp --optim onecycle --num_epochs 100 --dataset cifar10 --actfun $ACTFUN --mix_pre_apex --aug --var_n_params new
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model mlp --optim onecycle --num_epochs 100 --dataset cifar100 --actfun $ACTFUN --mix_pre_apex --aug --var_n_params new
