...
#SBATCH --output=logs/e5_overfit/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=e5_overfit
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model mlp --dataset $DATASET --actfun all_pk_comb_relu --overfit
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model cnn --dataset $DATASET --actfun all_pk_comb_relu --overfit
