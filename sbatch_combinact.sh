...
#SBATCH --array=0-19%10                      # array value
#SBATCH --output=logs/th1_nparam7_cnn/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=th1_nparam7_cnn
...
#python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model mlp --optim onecycle --num_epochs 100 --dataset $DATASET --actfun $ACTFUN --aug --var_n_params new
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --model cnn --optim onecycle --num_epochs 100 --dataset $DATASET --actfun $ACTFUN --aug --var_n_params new
