...
#SBATCH --array=0-1%2                         # array value
#SBATCH --output=logs/lr_range_others/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=lr_range_others
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_DIR --optim onecycle --num_epochs 56 --model resnet --resnet_width $RN_WIDTH --dataset $DATASET --actfun $ACTFUN --validation --lr_range --p 2 --perm_method invert --label _inv
