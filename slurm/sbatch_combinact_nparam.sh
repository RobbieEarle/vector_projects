...
#SBATCH --output=logs_new/wrn_50_nparam_test/%a-%N-%j    # %N for node name, %j for jobID
#SBATCH --job-name=wrn_50_nparam_test
...
SAVE_PATH=~/vector_projects/outputs/wrn_50_nparam
...
python engine.py --seed $SEED --save_path $SAVE_PATH --check_path $CHECK_PATH --model resnet --batch_size 128 --actfun $ACTFUN --resnet_width $RESNET_WIDTH --optim onecycle --num_epochs 100 --dataset cifar100 --aug --mix_pre_apex --num_params $NPARAMS
