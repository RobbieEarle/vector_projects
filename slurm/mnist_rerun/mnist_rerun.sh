...
#SBATCH --array=0-39%40                 # array value (for running multiple seeds, etc)
#SBATCH --output=logs/mnist_rerun/%x_%A-%a_%n-%t.out
#SBATCH --job-name=mnist_rerun
#SBATCH --qos=normal
...
echo ""
# Input handling
SAVE_PATH=~/vector_projects/outputs/mnist_rerun/
EPOCHS=100
...
python engine.py --seed "$SEED" --save_path "$SAVE_PATH" --check_path "$CKPT_DIR" --model cnn --actfun_idx $ACTFUN_IDX --optim onecycle --num_epochs $EPOCHS --dataset "mnist" --aug --label mnist_${NUM_PARAMS}_${ACTFUN_IDX} "${@:5}"
echo ""
