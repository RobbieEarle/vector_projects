...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        self.layer_dists.append({'inputs': x})
        x = self.linear_layers['l1'](x)
        self.layer_dists[0]['pre_act'] = x
        x = self.batch_norms['l1'](x) if not self.iris else x
        self.layer_dists[0]['pre_act_bn'] = x
        x = self.activate(x, 0)
        x = x.unsqueeze(0) if len(x.shape) == 1 else x
        self.layer_dists[0]['activations'] = x
        ...
        x = all_outputs
        self.layer_dists[1]['pre_act'] = x
        x = self.batch_norms['l2'](x) if not self.iris else x
        self.layer_dists[1]['pre_act_bn'] = x
        x = self.activate(x, 1)
        x = x.unsqueeze(0) if len(x.shape) == 1 else x
        self.layer_dists[1]['activations'] = x

        self.layer_dists.append({'inputs': x})
        x = self.linear_layers['l3'](x)
        self.layer_dists[2]['outputs'] = x
